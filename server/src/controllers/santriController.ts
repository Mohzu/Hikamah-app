...
export const uploadPhoto = async (req, res) => {
  ...
  console.log("ID Santri yang digunakan:", id_santri);

  // 2. Log dan validasi file yang diunggah
  console.log("Objek req.file:", req.file);
  if (!req.file) {
    return res
      .status(400)
  ...
  try {
    // 3. Ambil path foto lama dan hapus
    ...
    if (oldData.length > 0 && oldData[0].foto_profil) {
      const oldPhotoPath = path.join(
        ...
        "public",
        "uploads",
        path.basename(oldData[0].foto_profil)
      );
...
export const getFullProfile = async (req: Request, res: Response) => {
  try {
    ...
    const querySantri = `
            ...
            WHERE s.id = ?
        `;
    const [santriData] = await pool.query<RowDataPacket[]>(querySantri, [
      id_santri,
    ]);
    ...
    const [orangTuaData] = await pool.query<RowDataPacket[]>(
...
export const getJadwalPelajaran = async (
  ...
  try {
    ...
    const queryJadwal = `
        ...
        JOIN pengguna p ON g.id_pengguna = p.id  -- <-- Tambahkan JOIN ini
        WHERE jm.id_kelas = ?
