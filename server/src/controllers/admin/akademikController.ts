...
        const [validasi] = await pool.query<RowDataPacket[]>(`
            SELECT g.id AS id_guru, mp.id AS id_mapel, k.id AS id_kelas
            FROM guru g, mata_pelajaran mp, kelas k
            WHERE g.id = ? AND mp.id = ? AND k.id = ?
        `, [id_guru, id_mapel, id_kelas]);

        if (validasi.length === 0) {
            return res.status(404).json({ success: false, error: "Satu atau lebih ID (guru, mapel, kelas) tidak ditemukan." });
        }

        // Cek apakah mapel ini sudah ditambahkan ke kelas di kurikulum
        const [kurikulumCheck] = await pool.query<RowDataPacket[]>('SELECT id FROM kurikulum WHERE id_mapel = ? AND id_jenjang = (SELECT id_jenjang FROM kelas WHERE id = ?)', [id_mapel, id_kelas]);
        if (kurikulumCheck.length === 0) {
            return res.status(403).json({ success: false, error: "Mata pelajaran ini belum ditambahkan ke kurikulum jenjang kelas ini." });
        }
        
