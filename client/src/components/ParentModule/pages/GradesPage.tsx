...
import { ChevronDown, Download, CheckCircle, XCircle, Award } from 'lucide-react';
...
    // Mengambil data rapor dari API
    const fetchRaporData = async (tahunAjaran: string, semester: string) => {
        setIsLoading(true);
        setError(null);
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/api/account/santri/rapor?tahun_ajaran=${tahunAjaran}&semester=${semester}`,
...
    };
    
    // Mengambil daftar periode dan memuat rapor awal
    useEffect(() => {
        const fetchInitialData = async () => {
            setIsLoading(true);
            setError(null);
            try {
                // Perbaikan: Mengubah URL API agar sesuai dengan rute server
                const periodsResponse = await axios.get(
                    `${import.meta.env.VITE_API_URL}/api/account/santri/available-academic-years`,
                    { withCredentials: true }
                );
                
...
                    fetchRaporData(latestPeriod.tahun_ajaran, latestPeriod.semester);
...
    // Menggunakan useEffect terpisah untuk memuat rapor saat periode berubah
    useEffect(() => {
      if (selectedTahunAjaran && selectedSemester) {
        fetchRaporData(selectedTahunAjaran, selectedSemester);
      }
    }, [selectedTahunAjaran, selectedSemester]);

    const handleTahunAjaranChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      const newTahunAjaran = e.target.value;
      setSelectedTahunAjaran(newTahunAjaran);
      const newSemesters = availablePeriods.filter(p => p.tahun_ajaran === newTahunAjaran).map(p => p.semester);
...
                <div>
                    <h1 className="text-3xl font-extrabold mb-2">Rapor Santri</h1>
                    <p className="text-teal-100">
                        Ringkasan nilai dan prestasi akademik
                    </p>
                </div>
...
                    <div className="flex space-x-4">
                        <div className="relative">
                            <select 
                                ...
                                disabled={availableTahunAjaran.length === 0}
                            >
...
            
            {!raporData ? (
                <div className="p-4 text-center text-gray-500">Data rapor belum tersedia.</div>
            ) : (
                <>
                    {/* Rekapitulasi Umum */}
